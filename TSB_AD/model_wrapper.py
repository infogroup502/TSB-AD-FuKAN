...
Semisupervise_AD_Pool = ['Left_STAMPi', 'SAND', 'MCD', 'Sub_MCD', 'OCSVM', 'Sub_OCSVM', 'AutoEncoder', 'CNN', 'LSTMAD', 'TranAD', 'USAD', 'OmniAnomaly', 'PatchTST',
                        'AnomalyTransformer', 'TimesNet', 'FITS', 'FuKAN', 'FuKAN_v2', 'FuKAN_v3', 'FuKAN_P', 'FuKAN_best_42', 'FuKAN_best_1234', 'FuKAN_e10', 'FuKAN_e15', 'FuKAN_e20', 'Donut', 'OFA', 'MOMENT_FT', 'M2N2', 'TSPulse_FT', 'xLSTMAD', 'CHARM', 'StreamVAE', 'HSF', 'PaAno_PAI', 'SHADE', 'TimeRCD_MAFT']
...
def run_FuKAN(data_train, data_test, win_size=20, seq_len=2,
              local_size=(1,), global_size=(10,), fuzzy_sets=4, top_k=3,
              batch_size=128, epochs=2, lr=1e-4, device='auto',
              value_weight=1.0, normalize_mode='window', score_agg='tail',
              score_smooth=0.0, scheduler_type='step',
              next_step_weight=0.0):
    from .models.FuKAN import FuKAN
    clf = FuKAN(
        win_size=win_size, seq_len=seq_len,
        local_size=local_size, global_size=global_size,
        fuzzy_sets=fuzzy_sets, top_k=top_k,
        batch_size=batch_size, epochs=epochs, lr=lr, device=device,
        value_weight=value_weight,
        normalize_mode=normalize_mode, score_agg=score_agg,
        score_smooth=score_smooth, scheduler_type=scheduler_type,
...
def run_FuKAN_v2(data_train, data_test, **kwargs):
    """Alias for the tuned FuKAN (epochs=30 + global + max + value_weight=1).

    Kept as a separate pool name so that the old FuKAN baselines remain in
    the leaderboard while new runs are collected under ``FuKAN_v2``.
    """
    return run_FuKAN(data_train, data_test, **kwargs)

def run_FuKAN_v3(data_train, data_test, **kwargs):
    """Alias for the further-tuned FuKAN (cosine scheduler + global_size=10).

    Kept as a separate pool name so v2 and v3 results can be compared
    directly on the leaderboard.
    """
    return run_FuKAN(data_train, data_test, **kwargs)

def run_FuKAN_P(data_train, data_test, **kwargs):
    """FuKAN with the B4 next-step prediction head enabled (next_step_weight>0).

    Kept as a separate pool name so the prediction-augmented variant can be
    compared against plain FuKAN_v2 on the same files.
    """
    return run_FuKAN(data_train, data_test, **kwargs)

def run_FuKAN_best_42(data_train, data_test, **kwargs):
    """Final FuKAN version, seed 42 (highest VUS-PR main metric).

    Kept as a dedicated pool name so the final single-seed run is recorded
    under its own banner in the leaderboard.
    """
    return run_FuKAN(data_train, data_test, **kwargs)

def run_FuKAN_best_1234(data_train, data_test, **kwargs):
    """Final FuKAN version, seed 1234 (balanced top across all metrics).
...
def run_FuKAN_e10(data_train, data_test, **kwargs):
    """Ablation variant: FuKAN with epochs=10 (for epoch necessity study)."""
    return run_FuKAN(data_train, data_test, **kwargs)

def run_FuKAN_e15(data_train, data_test, **kwargs):
    """Ablation variant: FuKAN with epochs=15 (for epoch necessity study)."""
    return run_FuKAN(data_train, data_test, **kwargs)

def run_FuKAN_e20(data_train, data_test, **kwargs):
    """Ablation variant: FuKAN with epochs=20 (for epoch necessity study)."""
    return run_FuKAN(data_train, data_test, **kwargs)
